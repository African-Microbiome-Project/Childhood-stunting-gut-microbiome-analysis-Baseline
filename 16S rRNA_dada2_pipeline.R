...
path <- "/mnt/lustre/users/nkambunga/combined_16S"
...
saveRDS(seq_table_no_chim, file = file.path(path, "seq_table_no_chim.rds"))

# Assign taxonomy using the SILVA nr99 database
silva_nr99_w_species <- "/mnt/lustre/users/nkambunga/combined_16S/silva_nr99_v138.1_wSpecies_train_set.fa.gz"  # Update this path
taxa <- assignTaxonomy(seq_table_no_chim, silva_nr99_w_species, multithread = TRUE)

# Save the taxonomy assignment
saveRDS(taxa, file = file.path(path, "taxa_assigned.rds"))

# Optionally, save the taxonomy table in a CSV file
taxa_df <- as.data.frame(taxa)
write.csv(taxa_df, file = file.path(path, "taxa_assigned.csv"))
...
saveRDS(track, file = file.path(path, "track_reads.rds"))
write.csv(track, file = file.path(path, "track_reads.csv"))
